...
data=pd.read_csv(r'/Users/amberjones/Documents/IRdata/usefulpositionmcresults_10000.csv', delimiter=';', header=0) #download the file and put the path here
...
detector_pos=[np.array([0, 152, 1000]),np.array([-152, 0, 1000]),np.array([0, -152, 1000]),np.array([152, 0, 1000]),np.array([-135.057, 135.057, 300]),np.array([135.057, 135.057, 300]),np.array([135.057, -135.057, 300]),np.array([-135.057, -135.057, 300])]
m=-400 #bad angle
...
scatter_channel = [] #channel number for event
scatter2_channel = []
absorb_channel = []
lost=[]
                ...
                if data.iloc[coincidence_index+1,1]==j: #if the second detector involved was detector j
                    ...
                    if min_angles[i][j]<angle_scatterer_1<max_angles[i][j]: #if the angle from i to j is within the geometrically allowed values
                        if np.abs(E0+E1-Ei)<gap: #if scatter-absorption
                            ...
                            scatter_channel.append(data.iloc[coincidence_index,1])
                            ...
                            absorb_channel.append(data.iloc[coincidence_index + 1,1])
                            ...
                            scatter2_channel.append(data.iloc[coincidence_index + 1,1])
                        ...
                        if np.abs(E0+E1-Ei)<gap:
                            ...
                            scatter_channel.append(data.iloc[coincidence_index + 1,1])
                            ...
                            absorb_channel.append(data.iloc[coincidence_index, 1])
                        else:
                            ...
                            scatter2_channel.append(data.iloc[coincidence_index,1])
                    else:
                        lost.append(coincidence_index)


# dictionary of lists
dict = {'scatter energy': scattera_energies, 'scatter locations': scattera_locations, 'absorption energies': absorption_energies, 'absorption locations': absorption_locations, 'scatter channel': scatter_channel, 'absorbtion channel': absorb_channel}
...
df.to_parquet('/Users/amberjones/Documents/IRdata/dataprocessed10000')
